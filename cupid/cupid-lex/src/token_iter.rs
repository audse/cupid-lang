...
impl<'tokens> TokenListBuilder<'tokens> {
    ...
    fn next(
        self,
        next_fn: impl Fn(Self) -> (Option<Token<'static>>, Self),
        optional: IsOptional,
    ) -> Option<Self> {
        match next_fn(self) {
            (Some(token), mut next) => {
    ...
    pub fn one_of<S: Into<Vec<&'static str>>>(
        mut self,
        string: S,
        optional: IsOptional,
    ) -> Option<Self> {
        ...
        for item in string.into() {
            if (&self.tokens).0.peek(0)?.source == item {
                fulfilled = true;
            }
            self = self.string(item, Optional)?;
        }
        if fulfilled || optional == Optional {
            Some(self)
        } else {
            None
        }
    ...
    pub fn next_is_not(self, string: &'static str) -> Option<Self> {
        if let Some(token) = self.tokens.0.peek(0) && &*token.source != string {
            Some(self)
        } else {
            None
        }
    ...
    pub fn any_but(self, string: &'static str, optional: IsOptional) -> Option<Self> {
        self.next(
            |builder| (builder.tokens.any_but(string), builder),
            optional,
        )
    }
    ...
    pub fn repeat(
        mut self,
        condition: impl Fn(&Token<'static>) -> bool,
        mut inner: impl FnMut(Self) -> Option<Self>,
    ) -> Option<Self> {
        loop {
    ...
    pub fn repeat_collect<T>(
        self,
        condition: impl Fn(&Token<'static>) -> bool,
        mut inner: impl FnMut(Self) -> Option<(T, Self)>,
        with_result: impl FnOnce(Vec<T>),
        sep: Option<&'static str>,
    ) -> Option<Self> {
        let mut items: Vec<T> = vec![];
        let builder = self.repeat(condition, |builder| {
    ...
    pub fn list_collect<T>(
        self,
        delim: [&'static str; 2],
        inner: impl FnMut(Self) -> Option<(T, Self)>,
        with_result: impl FnOnce(Vec<T>),
        sep: Option<&'static str>,
    ) -> Option<Self> {
        self.string(delim[0], Required)?
            .repeat_collect(|token| token.source != delim[1], inner, with_result, sep)?
            .string(delim[1], Required)
    }

    pub fn paren_list(
        self,
        inner: impl FnMut(Self) -> Option<Self>,
        sep: &'static str,
    ) -> Option<Self> {
        self.list(PARENS, inner, sep)
    }

    pub fn paren_list_collect<T>(
        self,
        inner: impl FnMut(Self) -> Option<(T, Self)>,
        with_result: impl FnOnce(Vec<T>),
        sep: Option<&'static str>,
...
impl TokenIterator {
    ...
    pub fn advance_if(
        &mut self,
        closure: impl Fn(&Token<'static>) -> bool,
    ) -> Option<Token<'static>> {
