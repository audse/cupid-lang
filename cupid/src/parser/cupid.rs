...
use_item!(&mut node, self._expression(None), false);
		...
		pub fn _typed_declaration(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
			if let Some(memo) = self.memoize("typed_declaration".to_string(), None) {
				return Some(memo);
			}
			let start_pos = self.tokens.index();
			let pos = start_pos;
			...
			loop { 
use_item!(&mut node, self._boolean_declaration(None), false);

			let result = Some((node, true));
			return self.make_memo(start_pos, "typed_declaration".to_string(), result);
		
}
		self.reset_parse(&mut node, pos);
loop { 
use_item!(&mut node, self._integer_declaration(None), false);
		...
		self.reset_parse(&mut node, pos);
loop { 
use_item!(&mut node, self._decimal_declaration(None), false);
		...
		self.reset_parse(&mut node, pos);
loop { 
use_item!(&mut node, self._string_declaration(None), false);
		...
		self.reset_parse(&mut node, pos);
loop { 
use_item!(&mut node, self._function_declaration(None), false);
		...
		self.reset_parse(&mut node, pos);
			self.make_memo(start_pos, "typed_declaration".to_string(), None);
			None
		}
		
		pub fn _boolean_declaration(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
			if let Some(memo) = self.memoize("boolean_declaration".to_string(), None) {
				return Some(memo);
			}
			let start_pos = self.tokens.index();
			let pos = start_pos;
			let mut node = Node {
				name: "boolean_declaration".to_string(),
				tokens: vec![],
				children: vec![],
			};
			loop { 
use_item!(&mut node, self.expect("boo".to_string()), true);
use_optional!(&mut node, self.expect("mut".to_string()), false);
use_item!(&mut node, self._identifier(None), false);
loop { 
use_item!(&mut node, self.expect("=".to_string()), true);
...
break}
			let result = Some((node, false));
			return self.make_memo(start_pos, "boolean_declaration".to_string(), result);
		
}
		self.reset_parse(&mut node, pos);
			self.make_memo(start_pos, "boolean_declaration".to_string(), None);
			None
		}
		
		pub fn _integer_declaration(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
			if let Some(memo) = self.memoize("integer_declaration".to_string(), None) {
				return Some(memo);
			}
			let start_pos = self.tokens.index();
			let pos = start_pos;
			...
			loop { 
use_item!(&mut node, self.expect("int".to_string()), true);
use_optional!(&mut node, self.expect("mut".to_string()), false);
use_item!(&mut node, self._identifier(None), false);
loop { 
use_item!(&mut node, self.expect("=".to_string()), true);
...
break}
			let result = Some((node, false));
			return self.make_memo(start_pos, "integer_declaration".to_string(), result);
		
}
		self.reset_parse(&mut node, pos);
			self.make_memo(start_pos, "integer_declaration".to_string(), None);
			None
		}
		
		pub fn _decimal_declaration(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
			if let Some(memo) = self.memoize("decimal_declaration".to_string(), None) {
				return Some(memo);
			}
			let start_pos = self.tokens.index();
			let pos = start_pos;
			let mut node = Node {
				name: "decimal_declaration".to_string(),
				tokens: vec![],
				children: vec![],
			};
			loop { 
use_item!(&mut node, self.expect("dec".to_string()), true);
use_optional!(&mut node, self.expect("mut".to_string()), false);
use_item!(&mut node, self._identifier(None), false);
loop { 
use_item!(&mut node, self.expect("=".to_string()), true);
...
break}
			let result = Some((node, false));
			return self.make_memo(start_pos, "decimal_declaration".to_string(), result);
		
}
		self.reset_parse(&mut node, pos);
			self.make_memo(start_pos, "decimal_declaration".to_string(), None);
			None
		}
		
		pub fn _string_declaration(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
			if let Some(memo) = self.memoize("string_declaration".to_string(), None) {
				return Some(memo);
			}
			let start_pos = self.tokens.index();
			let pos = start_pos;
			let mut node = Node {
				name: "string_declaration".to_string(),
				tokens: vec![],
				children: vec![],
			};
			loop { 
use_item!(&mut node, self.expect("str".to_string()), true);
use_optional!(&mut node, self.expect("mut".to_string()), false);
use_item!(&mut node, self._identifier(None), false);
loop { 
use_item!(&mut node, self.expect("=".to_string()), true);
...
break}
			let result = Some((node, false));
			return self.make_memo(start_pos, "string_declaration".to_string(), result);
		
}
		self.reset_parse(&mut node, pos);
			self.make_memo(start_pos, "string_declaration".to_string(), None);
			None
		}
		
		pub fn _function_declaration(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
			if let Some(memo) = self.memoize("function_declaration".to_string(), None) {
				return Some(memo);
			}
			let start_pos = self.tokens.index();
			let pos = start_pos;
			...
			loop { 
use_item!(&mut node, self.expect("fun".to_string()), true);
use_optional!(&mut node, self.expect("mut".to_string()), false);
use_item!(&mut node, self._identifier(None), false);
loop { 
use_item!(&mut node, self.expect("=".to_string()), true);
...
break}
			let result = Some((node, false));
			return self.make_memo(start_pos, "function_declaration".to_string(), result);
		
}
		self.reset_parse(&mut node, pos);
			self.make_memo(start_pos, "function_declaration".to_string(), None);
			None
		}
		
		pub fn _equal(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
			if let Some(memo) = self.memoize("equal".to_string(), None) {
				return Some(memo);
			}
			let start_pos = self.tokens.index();
			let pos = start_pos;
...
use_negative_lookahead!(self, self.tokens.index(), &mut self.expect(">".to_string()));
			...
			let result = Some((node, true));
			return self.make_memo(start_pos, "equal".to_string(), result);
		
}
		self.reset_parse(&mut node, pos);
			self.make_memo(start_pos, "equal".to_string(), None);
			None
		}
...
use_item!(&mut node, self.expect(",".to_string()), true);
		...
		pub fn _log(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
			if let Some(memo) = self.memoize("log".to_string(), None) {
				return Some(memo);
			}
			let start_pos = self.tokens.index();
			let pos = start_pos;
...
use_item!(&mut node, self._closing_paren(None), false);
			...
			let result = Some((node, false));
			return self.make_memo(start_pos, "log".to_string(), result);
		
}
		self.reset_parse(&mut node, pos);
			self.make_memo(start_pos, "log".to_string(), None);
			None
		}
...
use_item!(&mut node, self.expect(",".to_string()), true);
		...
		pub fn _log_keyword(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
			if let Some(memo) = self.memoize("log_keyword".to_string(), None) {
				return Some(memo);
			}
			let start_pos = self.tokens.index();
			let pos = start_pos;
...
use_item!(&mut node, self.expect("log".to_string()), false);
			...
			let result = Some((node, true));
			return self.make_memo(start_pos, "log_keyword".to_string(), result);
...
use_item!(&mut node, self.expect("logs_line".to_string()), false);
		...
		self.reset_parse(&mut node, pos);
			self.make_memo(start_pos, "log_keyword".to_string(), None);
			None
		}
