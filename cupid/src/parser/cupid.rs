...
impl Parser {
    ...
    pub fn _typed_declaration(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
        ...
        self.reset_parse(&mut node, pos);
        loop {
            use_item!(&mut node, self._list_declaration(None), false);

            return Some((node, true));
        }
        self.reset_parse(&mut node, pos);
        loop {
            use_item!(&mut node, self._dictionary_declaration(None), false);
    ...
    pub fn _list_declaration(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
        let start_pos = self.tokens.index();
        let pos = start_pos;
        let mut node = Node {
            name: "list_declaration".to_string(),
            tokens: vec![],
            children: vec![],
        };
        loop {
            use_item!(&mut node, self.expect("list".to_string()), true);
            use_optional!(&mut node, self.expect("mut".to_string()), false);
            use_item!(&mut node, self._identifier(None), false);
            loop {
                use_item!(&mut node, self.expect("=".to_string()), true);
    ...
    pub fn _dictionary_declaration(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
        let start_pos = self.tokens.index();
        let pos = start_pos;
        let mut node = Node {
            name: "dictionary_declaration".to_string(),
            tokens: vec![],
            children: vec![],
        };
        loop {
            use_item!(&mut node, self.expect("dict".to_string()), true);
            use_optional!(&mut node, self.expect("mut".to_string()), false);
            use_item!(&mut node, self._identifier(None), false);
            loop {
                use_item!(&mut node, self.expect("=".to_string()), true);
    ...
    pub fn _require_property(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
        ...
        loop {
            use_item!(
                &mut node,
                self.expect_tag("<e 'missing index or property name'>".to_string()),
                false
            );
    ...
    pub fn _require_expression(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
        ...
        loop {
            use_item!(
                &mut node,
                self.expect_tag("<e 'missing expression'>".to_string()),
                false
            );
    ...
    pub fn _closing_paren(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
        ...
        loop {
            use_item!(
                &mut node,
                self.expect_tag("<e 'missing closing parenthesis'>".to_string()),
                false
            );
    ...
    pub fn _closing_brace(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
        ...
        loop {
            use_item!(
                &mut node,
                self.expect_tag("<e 'missing closing brace'>".to_string()),
                false
            );
    ...
    pub fn _closing_bracket(&mut self, _arg: Option<Token>) -> Option<(Node, bool)> {
        ...
        loop {
            use_item!(
                &mut node,
                self.expect_tag("<e 'missing closing bracket'>".to_string()),
                false
            );
