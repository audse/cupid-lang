...
impl PropertyNode {
	fn create_self_symbol(&self, function: &FunctionNode, value: &ValueNode, scope: &mut RLexicalScope) -> Result<ValueNode, Error> {
		let self_symbol = &function.params.symbols[0].symbol;
		let declare = RSymbolValue::Declaration { 
	...
	fn resolve_implementation_function(&self, left: &ValueNode, function_call: &FunctionCallNode, scope: &mut RLexicalScope) -> Option<Result<ValueNode, Error>> {
	...
	fn resolve_map_function(&self, left: &ValueNode, function_call: &FunctionCallNode, scope: &mut RLexicalScope) -> Option<Result<ValueNode, Error>> {
	...
	fn resolve_symbol_property(&self, left: &ValueNode, right: &SymbolNode, scope: &mut RLexicalScope) -> Result<ValueNode, Error> {
		match &left.value {
