...
fn test_is_defined() -> Result<(), Error> {
    ...
    assert!(get_ident("x", &mut env).is_some());
    Ok(())
}
...
fn test_namespaced_id() -> Result<(), Error> {
    ...
    type_def.value.methods.push(fun_decl("add", fun(["left", "right"])));
    ...
    type_def = type_def.resolve_names(&mut env)?;
    ident = ident.resolve_names(&mut env)?;
    // assert!()
    Ok(())
}